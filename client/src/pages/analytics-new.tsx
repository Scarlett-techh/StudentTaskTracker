...
export default function Analytics() {
  ...
  const tasksBySubject = completedTasks.reduce((acc: Record<string, any[]>, task: any) => {
  ...
  const subjectDistribution = Object.keys(tasksBySubject).map(subject => ({
    ...
    color: getSubjectColor(subject, subjects as any[])
  }));
  ...
  const favoriteSubject = subjectPerformance.length > 0 ? subjectPerformance.reduce(
  ...
  function getSubjectColor(subjectName: string, subjects: any[]) {
    if (!subjects || !Array.isArray(subjects)) return '#cccccc';
  ...
  const generateParentInsights = () => {
    ...
    setTimeout(() => {
      ...
      toast({
        ...
        description: "Updated learning insights are now available in the report.",
      });
    }, 1500);
  };
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {completedTasks.length > 0 && Array.isArray(tasks)
                  ? `${Math.round((completedTasks.length / Math.max(1, tasks.length)) * 100)}% completion rate` 
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center">
                                <div 
                                  ...
                                  style={{ backgroundColor: getSubjectColor(subject, subjects as any[]) }}
                                />
        ...
        {activeTab === 'subjects' && (
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>Subject Performance</CardTitle>
                <CardDescription>Progress across different subjects with detailed analysis</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Array.isArray(subjects) && subjects.map((subject: any) => (
                <Card key={subject.id || subject.name} className="overflow-hidden">
                  <CardHeader className="pb-2 bg-gray-50" style={{ borderBottom: `3px solid ${subject.color}` }}>
                    <div className="flex justify-between items-center">
                      <CardTitle className="text-lg">{subject.name}</CardTitle>
                      <Badge variant="outline">{
                        tasksBySubject[subject.name]?.length || 0
                      } tasks</Badge>
                    </div>
                  </CardHeader>
                  <CardContent className="p-4">
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-gray-50 rounded-lg p-3 text-center">
                          ...
                          <p className="text-lg font-bold">
                            {tasksBySubject[subject.name]?.filter((t: any) => t.status === 'completed').length || 0}/{tasksBySubject[subject.name]?.length || 0}
                          </p>
                        </div>
                        ...
                        <div className="bg-gray-50 rounded-lg p-3 text-center">
                          ...
                          <p className="text-lg font-bold">
                            {(tasksBySubject[subject.name]?.filter((t: any) => t.status === 'completed').length || 0) * 10}
                          </p>
                        </div>
                      </div>
                      ...
                      {/* Learning Growth */}
                      <div>
                        <h4 className="text-sm font-medium mb-2">Learning Growth</h4>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div 
                            className="bg-green-500 h-2 rounded-full" 
                            style={{ 
                              width: `${Math.min(100, ((tasksBySubject[subject.name]?.length || 0) * 10))}%`
                            }}
                          ></div>
                        </div>
                      </div>
